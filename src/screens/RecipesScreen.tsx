...
export function RecipesScreen() {
  ...
  const { savedRecipes, toggleSavedRecipe, recipeApiKey, t } = useApp();
  const CATEGORIES = useMemo(() => categories(t), [t]);
  const [subTab, setSubTab] = useState<'discover' | 'saved'>('discover');
  ...
  const results = useMemo(() => {
    ...
    return list;
  }, [subTab, category, query, savedRecipes, recipeApiKey, apiResults]);
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <Text style={styles.emptyText}>
              {subTab === 'saved' ? t.recipes.emptySaved : t.recipes.emptySearch}
            </Text>
          )
        }
      />
